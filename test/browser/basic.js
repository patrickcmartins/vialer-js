...
test('[browser] <alice> I am logging in.', async(t) => {
    ...
    await Promise.all([
        alice.goto('http://localhost/voys/webview/', {waitUntil: 'networkidle0'}),
        bob.goto('http://127.0.0.1/voys/webview/', {waitUntil: 'networkidle0'}),
    ])
    await alice.waitFor('.greeting')
    await alice.screenshot({path: path.join(rootPath, '1. login.png')})

    await alice.type('input[name="username"]', settings.tests.integration.username)
    await alice.type('input[name="password"]', settings.tests.integration.password)
    ...
    test('[browser] <alice> I am going through the wizard.', async(_t) => {
        ...
        test('[browser] <alice> I am calling bob.', async(__t) => {
            ...
            test('[browser] <bob> alice is calling; accepting the call.', async(___t) => {
